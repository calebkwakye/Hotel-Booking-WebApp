...
lottie_coding = getLottieAnimation(
    "https://assets4.lottiefiles.com/packages/lf20_JXUInT.json")
...
with st.container():
    ...
    with left_column:
        ...
        wantsToBook = st.radio("Do you want to book now(required):", [
                               "No", "Yes"], horizontal=True)

        # If they rsvp yes, show the room types, number of people and the check-in/check-out dates
        if wantsToBook == "Yes":
            roomChoice = st.selectbox("Room type (required):", [
                                      "", "Junior Suite", "Executive Suite", "Super Deluxe"])
            numberOfAdults = st.selectbox(
                "Adults (required):", ["", 1, 2, 3, 4])
            numberOfChildren = st.selectbox(
                "Children (required):", ["", 1, 2, 3, 4])
        ...
        if (((username != "") and (email != "") and (wantsToBook == "No")) or
            ...
            if (bookButton == True):
                ...
                # Enter your address (also the username)
                sender_email = "booking.spiderhubhotel@gmail.com"
